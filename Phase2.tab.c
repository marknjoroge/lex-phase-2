...
#line 41 "Phase2.y"
                   {
                        if (findvarInScop((yyvsp[0].symp).name, curscope) == 1) 
                            semantic_error("Declaration Error: Variable already declared.");
                        else {
                            insertvar((yyvsp[0].symp).name, (yyvsp[-1].symp).name);
                            varcount++;
                        }
                    }
#line 1183 "Phase2.tab.c"
    break;

  case 15: /* assignment_stmt: ID ASG bexpression  */
#line 53 "Phase2.y"
                                     {
                        if (strcmp(findtype((yyvsp[-2].symp).name, curscope), "XXX") != 0) 
                            semantic_error("Initialision Error: Variable not declared.");
                        if (sizeof((yyvsp[-2].symp)) != sizeof((yyvsp[0].symp)))
                            semantic_error("Type Error: Types do not match");
                    }
#line 1194 "Phase2.tab.c"
    break;
...
#line 1218 "Phase2.tab.c"
    break;

  case 21: /* exp: exp PLUS exp  */
#line 78 "Phase2.y"
                     { 
                        if ((strcmp(findtype((yyvsp[-2].symp).name, curscope), "int") 
                                || strcmp(findtype((yyvsp[-2].symp).name, curscope), "float"))
                            && (strcmp(findtype((yyvsp[0].symp).name, curscope), "int") 
                                || strcmp(findtype((yyvsp[0].symp).name, curscope), "int")) )
                                
                            printf("All good");
                            // $$ = $1 + $3; 
                    }
#line 1232 "Phase2.tab.c"
    break;
...
#line 98 "Phase2.y"
                         { (yyval.symp) = (yyvsp[-1].symp);curscope++; }
#line 1252 "Phase2.tab.c"
    break;
...
#line 1288 "Phase2.tab.c"
    break;

  case 34: /* if_head: IF LPAREN bexpression RPAREN OP  */
#line 124 "Phase2.y"
                                          {
                        if (sizeof((yyvsp[-2].symp)) != 1) semantic_error("Type Error: Condition is not a Boolean.");
                    }
#line 1296 "Phase2.tab.c"
    break;

  case 35: /* while_stmt: WHILE LPAREN bexpression RPAREN OP statements CL  */
#line 128 "Phase2.y"
                                                              {
                        if (sizeof((yyvsp[-4].symp)) != 0) semantic_error("Type Error: Condition is not a Boolean.");
                    }
#line 1304 "Phase2.tab.c"
    break;
